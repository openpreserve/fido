  ...
  <soap:Envelope xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
    ...
    <soap:Body>
      <getSignatureFileVersionV1
        xmlns="http://pronom.nationalarchives.gov.uk" />
    </soap:Body>
  </soap:Envelope>'''.strip()
...
soap_file_header = (
    '"http://pronom.nationalarchives.gov.uk:getSignatureFileV1In"')
...
def get_pronom_signature(type_):
    ...
    try:
        if type_ == "version":
            soap_action = soap_version_header
            soap_str = soap_version_container
        elif type_ == "file":
            soap_action = soap_file_header
            soap_str = soap_file_container
        else:
        ...
        webservice.putheader("Content-length", "%d" % len(soap_str))
        ...
        if statuscode == 200:
            ...
            if type_ == "version":
                ...
                sys.stderr.write(
                    "get_pronom_signature(): could not parse VERSION from"
                    " SOAP response: " + type_)
                return False
            if type_ == "file":
                ...
                if len(sigtxt) > 0:
                    ...
                    if not check_well_formedness(tmpfile):
                        os.unlink(tmpfile)
                        sys.stderr.write(
                            "get_pronom_signature(): signaturefile not well"
                            " formed")
                        return False
                    os.unlink(tmpfile)
                    ...
                    sys.stderr.write(
                        "get_pronom_signature(): could not parse XML from SOAP"
                        " response: " + type_)
                    return False
        else:
            sys.stderr.write(
                "get_pronom_signature(): webservice error: '{} {}'\n".format(
